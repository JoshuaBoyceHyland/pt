...
PlayerState* IdlePlayerState::handleInput(gpp::Events& input) {
	...
	else if (input.getCurrent() == gpp::Events::Event::LIGHT_ATTACK_EVENT) {
		DEBUG_MSG("IdlePlayerState -> LightAttackPlayerState");
		return new LightAttackPlayerState();
	}

	else if(input.getCurrent() == gpp::Events::Event::KICK_ATTACK_EVENT){
		DEBUG_MSG("IdlePlayerState -> StompAttackPlayerState");
		return new KickAttackPlayerState(); 
	}
	else if(input.getCurrent() == gpp::Events::Event::SPECIAL_ABILITY_START_PLAYER_STATE_){
		DEBUG_MSG("IdlePlayerState -> SPECIAL_ABILITY_START_PLAYER_STATE");
		return new SpecialAbilityPlayerState(); 
	}
	else if(input.getCurrent() == gpp::Events::Event::JUMP_RUN_EVENT){
		DEBUG_MSG("IdlePlayerState -> SPECIAL_ABILITY_START_PLAYER_STATE");
		return new JumpPlayerState(); 
	}
